#!/usr/bin/env python3
"""
SubagentStop hook for checkpoint skill.

This hook verifies task completion before stopping a subagent.
...
LOG_FILE = os.path.expanduser("~/.claude/subagent-stop-debug.log")
...
def main():
    """
    Hook that runs when a subagent is about to stop.
    ...
    if not session_id or not agent_id:
        # Missing required fields, log and allow stopping
    ...
    if incomplete_transcript:
        ...
        response = {
            "decision": "block",
            "reason": f"<system-reminder>Tasks incomplete: {incomplete_transcript}. Complete all tasks before stopping.</system-reminder>"
        }
        ...
        response = {
            "decision": "block",
            "reason": f"<system-reminder>Tasks incomplete: {task_names}. Complete all tasks before stopping.</system-reminder>"
        }
    elif has_tasks:
