...
#
# Output:
#   On success: JSON array of labels with id, name, color
...
if [[ -n "$TEAM_ID" ]]; then
  QUERY='query IssueLabels($teamId: ID) { issueLabels(filter: { team: { id: { eq: $teamId } } }) { nodes { id name color description isGroup } } }'
  VARIABLES="{\"teamId\": \"$TEAM_ID\"}"
else
  QUERY='query IssueLabels { issueLabels { nodes { id name color description isGroup } } }'
  VARIABLES="{}"
fi
